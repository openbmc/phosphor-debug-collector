...
size_t Manager::getAllowedSize()
{
    ...
    log<level::INFO>(fmt::format("getAllowedSize: dumpDir={}",dumpDir).c_str());
    ...
    for (const auto& p : std::filesystem::recursive_directory_iterator(dumpDir))
    {
        if (!std::filesystem::is_directory(p))
        {
            ...
            log<level::INFO>(fmt::format("getAllowedSize: path={}",p.path().filename().string()).c_str());
        }
    }
    ...
    log<level::INFO>(fmt::format("getAllowedSize: BMC_DUMP_TOTAL_SIZE={}",BMC_DUMP_TOTAL_SIZE).c_str());
    log<level::INFO>(fmt::format("getAllowedSize: BMC_DUMP_MIN_SPACE_REQD={}",BMC_DUMP_MIN_SPACE_REQD).c_str());
