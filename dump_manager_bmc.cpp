    ...
    Manager::createDump(phosphor::dump::DumpCreateParams params)
{
    ...
    {
        lg2::warn("BMC dump accepts not more than 2 additional parameters");
    }
    ...
    {
        lg2::error(
            "Error in creating dump entry, errormsg: {ERROR_MSG}, OBJECTPATH: {OBJECT_PATH}, ID: {ID}",
                              ...
                              const std::vector<std::string>& fullPaths)
{
    ...
    if (pid == 0)
    {
        ...
        auto error = errno;
        lg2::error(
            "Error occurred during dreport function execution, errno: {ERROR_CODE}",
            "ERROR_CODE", error);
        elog<InternalFailure>();
    }
    else if (pid > 0)
    {
        ...
        catch (const sdeventplus::SdEventError& ex)
        {
            ...
            lg2::error(
                "Error occurred during the sdeventplus::source::Child creation ex: {EXCEPTION_MSG}",
...
void Manager::createEntry(const std::filesystem::path& file)
{
    ...
    {
        lg2::error(
            "Error in creating dump entry, errormsg: {ERROR_MSG}, OBJECTPATH: {OBJECT_PATH}, ID: {ID}, TIMESTAMP: {TIMESTAMP}, SIZE: {SIZE}, FILENAME: {FILENAME}",
            "ERROR_MSG", e, "OBJECT_PATH", objPath.c_str(), "ID", id,
