...
void Manager::createEntry(const std::filesystem::path& file)
{
    ...
    {
        lg2::error("Invalid Dump file name, FILENAME: {FILENAME}", "FILENAME", file);
        return;
    }
    ...
    try
    {
        ...
        createOrUpdateEntry(id, timestamp, std::filesystem::file_size(file), file,
...
std::filesystem::path Manager::createEntry(const uint32_t id, const uint64_t ms,
                          ...
                          OriginatorTypes originatorType)
{
    ...
    try
    {
        entries.emplace(
            id,
            std::make_unique<
                phosphor::dump::new_::Entry<sdbusplus::xyz::openbmc_project::Dump::Entry::server::BMC,
                      phosphor::dump::DumpEntryHelper>>(bus, objPath.c_str(), id, ms, fileSize,
...
std::filesystem::path Manager::createOrUpdateEntry(const uint32_t id, const uint64_t timestamp,
                          uint64_t fileSize, const std::filesystem::path& file,
                          phosphor::dump::OperationStatus status,
                          std::string originatorId,
                          OriginatorTypes originatorType)
{
    ...
    {
        return createEntry(id, timestamp, fileSize, file,
                    status, originatorId,
                    originatorType);
    }
