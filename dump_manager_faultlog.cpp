...
{
#ifdef FAULTLOG_MATCH_INTEL_CRASHDUMP
    crashdumpMatch = std::make_unique<sdbusplus::bus::match_t>(
        bus,
        sdbusplus::bus::match::rules::propertiesChangedNamespace(
            INTEL_CRASHDUMP_OBJPATH_NAMESPACE, INTEL_CRASHDUMP_INTERFACE) +
            sdbusplus::bus::match::rules::sender(INTEL_CRASHDUMP_BUSNAME),

        [this](sdbusplus::message_t& msg) {
            if (msg.is_method_error())
            {
                log<level::ERR>(
                    "dump_manager_faultlog got crashdump added error!");
                return;
            }
            ...
            for (const auto& [changedProp, newValue] : changedProps)
            {
                if (changedProp == "Log")
                {
                    ...
                    createDumpFromCrashdumpSignal(msg);
                }
            }
        });
#endif
...
{
#ifdef FAULTLOG_MATCH_BIOS_BMC_SMM_ERROR_LOGGER
    cperLogMatch = std::make_unique<sdbusplus::bus::match_t>(
        bus,
        sdbusplus::bus::match::rules::interfacesAdded() +
            sdbusplus::bus::match::rules::path_namespace(
                BIOS_BMC_SMM_ERROR_LOGGER_OBJPATH_NAMESPACE) +
            sdbusplus::bus::match::rules::sender(
                BIOS_BMC_SMM_ERROR_LOGGER_BUSNAME),
...
void Manager::createDumpFromCrashdumpSignal(sdbusplus::message_t& msg)
{
    std::string objpath = msg.get_path();

    std::map<std::string, std::variant<std::string, uint64_t>> crashdumpMap;

    crashdumpMap.insert(
        std::pair<std::string, std::string>("Type", "Crashdump"));

    crashdumpMap.insert(std::pair<std::string, std::string>(
        "PrimaryLogId", std::filesystem::path(objpath).filename()));
