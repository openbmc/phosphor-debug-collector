    ...
    Manager::createDump(phosphor::dump::DumpCreateParams params)
{
    ...
    log<level::INFO>(
        fmt::format("dump_manager_faultlog.cpp: id = {}, entries.size() = {}",
                    id, entries.size())
            .c_str());
    ...
    try
    {
        ...
        entries.insert(std::make_pair(
            ...
            std::make_unique<faultlog::Entry>(
                ...
                phosphor::dump::OperationStatus::Completed, entryType,
                primaryLogIdStr, *this, &entries)));

        log<level::INFO>(
            fmt::format("dump_manager_faultlog.cpp newEntryId: {}", id)
                .c_str());
    }
...
void Manager::saveEarliestEntry()
{
    ...
    {
        log<level::INFO>(
            fmt::format("dump_manager_faultlog.cpp: save "
                        "entry. type = {}, savedEntries->size() = {}",
        ...
        if (savedEntries->size() < maxNumSavedEntries)
        {
            ...
            log<level::INFO>(fmt::format("dump_manager_faultlog.cpp: "
                                         "savedEntries->size() after add = {}",
                                         savedEntries->size())
                                 .c_str());
        }
        else
    ...
    log<level::INFO>(
        fmt::format(
            "dump_manager_faultlog.cpp: entries.size() after erase = {}",
            entries.size())
            .c_str());
}
