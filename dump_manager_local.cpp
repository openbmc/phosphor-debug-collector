    ...
    Manager::createDump(std::map<std::string, std::string> params)
{
    ...
    try
    {
        ...
        entries.insert(std::make_pair(
            id, std::make_unique<local::Entry>(
...
void Manager::createEntry(const fs::path& file)
{
    ...
    std::regex file_regex(dumpFileRegex);
    std::smatch match;
    ...
    {
        dynamic_cast<phosphor::dump::local::Entry*>(dumpEntry->second.get())
    ...
    for (const auto& i : fileInfo)
    {
        ...
        else if ((IN_CREATE == i.second) && fs::is_directory(i.first))
        {
            auto watchObj = std::make_unique<Watch>(
                ...
                std::bind(
                    std::mem_fn(&phosphor::dump::local::Manager::watchCallback),
