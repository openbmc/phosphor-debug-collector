...
Watch::Watch(sd_event* loop)
{
    ...
    {
        elog<InvalidCorePath>(xyz::openbmc_project::Dump::Monitor::
                              InvalidCorePath::PATH("CORE_FILE_DIR"));
        return;
    }

    //Check for existing coredumps, Dump manager should handles this before
                    ...
                    void* userdata)
{
    ...
    while (offset < bytes)
    {
        ...
        if ((event->mask & IN_CLOSE_WRITE) && !(event->mask & IN_ISDIR))
        {
            ...
            corePaths.push_back(p /= event->name) ;
        }
