                    ...
                    std::string writePath)
{
    ...
    try
    {
        ...
        else if ((retVal > 0) && (FD_ISSET(unixSocket(), &readFD)))
        {
            ...
            if (!infile.good())
            {
                ...
                lg2::error(
                    "Failed to open the dump from file, errno: {ERRNO}, DUMPFILE: {DUMP_FILE}, DUMP_ID: {DUMP_ID}",
                    "ERRNO", errno, "DUMP_FILE", file.c_str(), "DUMP_ID",
                    dumpId);
    ...
    catch (const std::ifstream::failure& oe)
    {
        ...
        auto err = errno;
        lg2::error(
            "Failed to open, errormsg: {ERROR_MSG}, OPENINTERFACE: {OPEN_INTERFACE}, DUMP_ID: {DUMP_ID}",
            "ERROR_MSG", oe, "OPEN_INTERFACE", file.c_str(), "DUMP_ID", dumpId);
    ...
    catch (const std::exception& e)
    {
        ...
        auto err = errno;
        lg2::error(
            "Failed to offload dump, errormsg: {ERROR_MSG}, DUMPFILE: {DUMP_FILE}, DUMP_ID: {DUMP_ID}",
            "ERROR_MSG", e, "DUMP_FILE", writePath.c_str(), "DUMP_ID", dumpId);
