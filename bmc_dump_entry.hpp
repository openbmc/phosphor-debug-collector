...
class Entry : virtual public phosphor::dump::Entry, virtual public EntryIfaces
{
  public:
     ...
     */
    static std::unique_ptr<Entry>
        deserializeEntry(sdbusplus::bus_t& bus, uint32_t id,
                         const std::string& objPath,
                         const std::filesystem::path& filePath, Manager& parent)
    {
        try
        {
            auto entry = std::make_unique<Entry>(bus, objPath, id, filePath,
                                                 parent);
            entry->updateFromFile(filePath);
            entry->deserialize(filePath);
            entry->emitSignal();
            return entry;
        }
  ...
  private:
    Entry(sdbusplus::bus_t& bus, const std::string& objPath, uint32_t dumpId,
          const std::filesystem::path& file, Manager& parent) :
        phosphor::dump::Entry(bus, objPath, dumpId, 0, 0, file,
