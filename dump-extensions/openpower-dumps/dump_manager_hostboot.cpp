...
void Manager::notify(uint32_t dumpId, uint64_t)
{
    ...
    auto dumpPath =
        fs::path(HOSTBOOT_DUMP_TMP_FILE_DIR) / std::to_string(dumpId);
                              ...
                              uint32_t id)
{
    ...
    if (pid == 0)
    {
        ...
        execl("/usr/bin/dreport", "dreport", "-d", dumpPath.c_str(), "-i",
              ...
              "hostboot", "-n", "hostboot", nullptr);

        // dreport script execution is failed.
        auto error = errno;
        log<level::ERR>(
            "Hostboot Dump: Error occurred during dreport function execution",
            entry("ERRNO=%d", error));
        elog<InternalFailure>();
    }
    else if (pid > 0)
...
size_t Manager::getAllowedSize()
{
    ...
    if (size < HOSTBOOT_DUMP_MIN_SPACE_REQD)
    {
