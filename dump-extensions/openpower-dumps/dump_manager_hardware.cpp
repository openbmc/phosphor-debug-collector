    ...
    Manager::createDump(std::map<std::string, std::string> params)
{
    ...
    try
    {
        ...
        entries.insert(std::make_pair(
            id, std::make_unique<openpower::dump::hardware::Entry>(
                              ...
                              uint32_t id)
{
    ...
    if (pid == 0)
    {
        ...
        auto error = errno;
        log<level::ERR>(
            "Hardware Dump: Error occurred during opdreport function execution",
            entry("ERRNO=%d", error));
        elog<InternalFailure>();
    }
    else if (pid > 0)
    {
        ...
        if (0 > rc)
        {
            ...
            log<level::ERR>("Hardware Dump: Error occurred during the "
    ...
    {
        auto error = errno;
        log<level::ERR>("Hardware Dump: Error occurred during fork",
                        entry("ERRNO=%d", error));
        elog<InternalFailure>();
    }
