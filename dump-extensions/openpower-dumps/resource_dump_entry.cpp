...
void Entry::initiateOffload(std::string uri)
{
    ...
    lg2::info(
        "Resource dump offload request id: {ID} uri: {URI} source dumpid: {SOURCE_DUMP_ID}",
...
void Entry::delete_()
{
    ...
    {
        lg2::error(
            "Dump offload is in progress, cannot delete dump, id: {DUMP_ID} srcdumpid: {SRC_DUMP_ID}",
    ...
    if ((phosphor::dump::isHostRunning()) && (srcDumpID != INVALID_SOURCE_ID))
    {
        ...
        {
            lg2::error(
                "Error deleting dump from host id: {DUMP_ID} host id: {SRC_DUMP_ID} error: {ERROR_MSG}",
