...
void Manager::notify(uint32_t dumpId, uint64_t size)
{
    ...
    for (auto& entry : entries)
    {
        ...
        if ((resEntry->status() == phosphor::dump::OperationStatus::InProgress) &&
    ...
    Manager::createDump(std::map<std::string, std::string> params)
{
    ...
    auto idString = std::to_string(id);
    auto objPath = fs::path(baseEntryPath) / idString;
    std::string vspString = params[sdbusplus::com::ibm::Dump::server::Create::convertCreateParametersToString(CreateParameters::VSPString)];
    std::string pwd = params[sdbusplus::com::ibm::Dump::server::Create::convertCreateParametersToString(CreateParameters::Password)];
    entries.insert(std::make_pair(
        id, std::make_unique<resource::Entry>(
                bus, objPath.c_str(), id, 0, 0, INVALID_SOURCE_ID,
                std::string(), std::string(), phosphor::dump::OperationStatus::InProgress, *this)));
    lastEntryId++;
