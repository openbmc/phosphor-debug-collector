                   ...
                   const std::string& prop)
{
    ...
    catch (const std::exception& ex)
    {
        lg2::error(
            ...
            "object path: {OBJECT_PATH} error: {ERROR_MSG} ",
            "PROPERTY", prop.c_str(), "INTERFACE", intf.c_str(), "OBJECT_PATH",
            object.c_str(), "ERROR_MSG", ex);
        throw;
    }
    return retVal;
}
 ...
 *
 * @throws InternalFailure - If getting the state property fails.
 */
template <typename T>
                ...
                const std::string& state)
{
    try
    {
        ...
        std::string valueStr = std::get<std::string>(propertyVal);

        if constexpr (std::is_same_v<T, BootProgress>)
        {
            return sdbusplus::xyz::openbmc_project::State::Boot::server::
                Progress::convertProgressStagesFromString(valueStr);
        }
        else if constexpr (std::is_same_v<T, HostState>)
        {
            return sdbusplus::xyz::openbmc_project::State::server::Host::
                convertHostStateFromString(valueStr);
        }
    }
    catch (const sdbusplus::exception_t& e)
    {
        lg2::error(
            ...
            "INTERFACE: {INTERFACE}, PROPERTY: {PROPERTY}, error: {ERROR_MSG}",
            "OBJPATH", objPath, "INTERFACE", intf, "PROPERTY", state,
            "ERROR_MSG", e);
    ...
    catch (const std::bad_variant_access& e)
    {
        lg2::error(
            ...
            "value,  OBJPATH: {OBJPATH}, INTERFACE: {INTERFACE}, error: {ERROR_MSG}",
            "STATE", state, "OBJPATH", objPath, "INTERFACE", intf, "ERROR_MSG",
            e);
        elog<InternalFailure>();
    }
}
